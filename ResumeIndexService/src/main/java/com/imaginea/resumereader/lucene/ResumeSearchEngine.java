...
public class ResumeSearchEngine {
    ...
    private List<FileInfo> extractHits(ScoreDoc[] hits) throws IOException {
        ...
        for (int i = 0; i < hits.length; i++) {
            ...
            hitList.add(new FileInfo(d.getField(this.fileNameField).stringValue(), d.getField(this.personNameField).stringValue(),
                    d.getField(this.summaryField).stringValue()));
        }
        return hitList;
    }
}
